...
class ConfigManager:
    ...
    _config_dir = Path.home() / ".drews-xcode-mcp"
    # Written by the package's pre-rename releases; adopted via rename on first
    # run (see _migrate_legacy_config_dir).
    _legacy_config_dir = Path.home() / ".xcode-mcp-server"
        ...
        if cls._instance is None:
            with cls._instance_lock:
                if cls._instance is None:
                    ...
                    cls._migrate_legacy_config_dir()
    ...
    @classmethod
    def _migrate_legacy_config_dir(cls):
        """Adopt settings written by the pre-rename xcode-mcp-server releases.

        If ~/.drews-xcode-mcp does not exist yet but the old ~/.xcode-mcp-server
        does, rename it so there is exactly one live copy of the user's settings.
        Must run before anything creates the new directory, or the old settings
        would be stranded. A failed rename (permissions, concurrent process)
        falls back to a fresh directory rather than failing startup.
        """
        if cls._config_dir.exists():
            if cls._legacy_config_dir.exists():
                # A pre-rename xcode-mcp-server process (e.g. a cached uvx
                # build still referenced by another MCP client) can recreate
                # the old directory after migration, or a failed rename can
                # leave it behind. Its settings are never read; say so once
                # per process instead of diverging silently.
                print(
                    f"Note: {cls._legacy_config_dir} exists but is ignored; settings now live in "
                    f"{cls._config_dir}. It was likely recreated by an old 'xcode-mcp-server' "
                    f"install still configured in some MCP client — update that client to "
                    f"'drews-xcode-mcp' and delete the old directory.",
                    file=sys.stderr,
                )
            return
        if not cls._legacy_config_dir.exists():
            return
        try:
            cls._legacy_config_dir.rename(cls._config_dir)
            print(f"Migrated settings from {cls._legacy_config_dir} to {cls._config_dir}", file=sys.stderr)
        except OSError as e:
            print(f"Warning: could not migrate {cls._legacy_config_dir} to {cls._config_dir}: {e}", file=sys.stderr)

    def _ensure_config_dir_exists(self):
        """Create ~/.drews-xcode-mcp if needed"""
    ...
    def _load_config_file(self, file_path: Path) -> Optional[dict]:
        ...
        try:
            ...
            print(f"Warning: Failed to load config from {file_path}: {e}", file=sys.stderr)
            return None
